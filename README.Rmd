...
[API](https://developers.tally.so). Authenticate with an API key, list your
...
If you work with several Tally accounts, store each account's key in its own
`TALLY_API_KEY_<NAME>` environment variable:
...
Then import its submissions as a wide tibble, with one row per submission
