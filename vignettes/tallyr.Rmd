---
title: "Get started with tallyr"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Get started with tallyr}
...
[API](https://developers.tally.so). It covers a simple but common workflow:
authenticate with an API key, list your forms, and import form submissions
as tidy tibbles, ready for analysis.

Because every function here talks to the Tally API, the code in this
vignette isn't run when the vignette is built; the output shown is from an
example account.
...
To confirm you're authenticated, run `tally_whoami()`:
...
If something isn't working, `tally_sitrep()` gives a full situation report
on your setup, with recommendations for fixing anything that's wrong --
see `vignette("troubleshooting")`.

If you have more than one Tally account, you can store a key for each and
switch between them -- see `vignette("multiple-accounts")`.

## Listing your forms

`tally_forms()` returns a tibble of all the forms you have access to, one
row per form:
...
`"PUBLISHED"` or `"DELETED"`), the tibble includes the `id` you'll need to
import its submissions. The same ID appears in the form's URL on tally.so,
so you can also read it off there directly.
...
`tally_submissions()` imports a form's submissions as a wide tibble: one
row per submission, one column per question, with question titles as the
column names:
...
The Tally API returns results in pages, and limits clients to 100 requests
per minute. You don't need to manage either: `tally_forms()` and
`tally_submissions()` fetch all pages automatically, and every request is
throttled to stay within the rate limit.

## Where next

- `vignette("submission-data")` looks closer at the tibble
  `tally_submissions()` returns and common patterns for tidying it.
- `vignette("multiple-accounts")` covers working with several Tally
  accounts in one session.
- `vignette("troubleshooting")` walks through diagnosing setup problems
  with `tally_sitrep()`.
